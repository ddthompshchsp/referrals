...
def write_excel(df, summary, author_fix, cols, cutoff_date):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        # --- Sheet 1 formatting + logo ONLY here ---
...
# ----------------------------
logo_path_ui = Path("header_logo.png")
hdr_l, hdr_c, hdr_r = st.columns([1, 2, 1])
with hdr_c:
    if logo_path_ui.exists():
        st.image(str(logo_path_ui), width=320)
    st.markdown(
